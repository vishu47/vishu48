...
export function IngestionWidget1({setFiles, files}: Props) {
  // provider
  // url
  // redux
  // states
  ...
  const onDrop = (acceptedFiles: any) => {
    // check for exixtance
    const Existance = files.find((file: any) => file.name === acceptedFiles[0].name)
    // gaurd
    if (Existance) return toast.warning(`${acceptedFiles[0].name} is already exist.`)

    // concat array from previous upload
    const concatArr = files.concat(
